...
class SHARKintData(object):
    ...
    def get_data_in_dataframe(self):
        ...
        df = pd.read_sql(self.get_sql_query(),
                         self._get_engine())
    ...
    def _get_engine(self):
        ...
        if self.prod_or_test == 'prod':
            engine = pyodbc.connect(**settings['servers'].get('sharkint_prd'))
        elif self.prod_or_test == 'test':
            engine = pyodbc.connect(**settings['servers'].get('sharkint_tst'))
        else:
            raise UserWarning('sharkint prod/test not set', self.prod_or_test)

        return engine


if __name__ == '__main__':
    si = SHARKintData('prod')
